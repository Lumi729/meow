...
export function mountPanel(markup, preferences, save){
 ...
 const defaults=()=>new URL(mode==='bad'?'assets/love.jpg':'assets/record-player.jpg',base).href;
 ...
 const refresh=()=>{topButton.hidden=preferences.top_enabled===false;floating.hidden=preferences.enabled===false;const size=Math.min(120,Math.max(36,Number(preferences.size)||64));floating.style.width=floating.style.height=`${size}px`;img.src=safeImage(mode==='bad'?preferences.bad_image:preferences.normal_image);if(Number.isFinite(preferences.x)&&Number.isFinite(preferences.y))place(preferences.x,preferences.y);else{place(innerWidth-size-12,innerHeight-size-110);}};
