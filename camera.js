...
export function mountCamera({root,context,chatKey,panel,page,secondary,run,isBusy,config,prepare,png,makeEntry,addImage,listImages,stop,getAppearance=async()=>''}){
 ...
 const generateTags=action('① 发送勾选镜头，生成 tags',()=>run(async signal=>{
  ...
  for(const record of chosen){
   ...
   valid(s);if(data.choices?.[0]?.finish_reason==='length')throw new Error('副 API 输出被截断，请缩短 tag 预设后重试。');const item=parseScenes(data.choices?.[0]?.message?.content,[record.id],1)[0];scenes.push(item);
 ...
 const receive=async e=>{
  ...
  let s;try{
   ...
   session=s;originalInput.value=s.snapshot;appearanceInput.value=await getAppearance();valid(s);scenes=[];tags.replaceChildren();cards.replaceChildren();selected=[];
