...
export async function init(){
 ...
 const renderConnections=()=>{const list=el('secondary-profile-list');list.replaceChildren(new Option('选择已保存的副 API',''));for(const p of ext.meow_secondary_profiles)list.add(new Option(p.name,p.id));list.value=ext.meow_secondary_profile_id||'';el('secondary-profile-delete').disabled=!list.value;};
 renderConnections();el('secondary-profile-name').value=ext.meow_secondary_profiles.find(p=>p.id===ext.meow_secondary_profile_id)?.name||'';
 on('secondary-profile-save',()=>{if(busy)throw new Error('请等待当前任务完成。');if(el('secondary-key').value.trim())throw new Error('请先点“保存副 API 密钥”，再保存配置。');const name=el('secondary-profile-name').value.trim().slice(0,80);if(!name)throw new Error('请填写配置名字。');apiBase(secondary.url);if(!secondary.model.trim()||!secondary.secret_id)throw new Error('请填写模型并保存副 API 密钥。');let p=ext.meow_secondary_profiles.find(p=>p.name===name);if(p&&!confirm(`覆盖副 API 配置“${name}”？`))return;if(!p){p={id:crypto.randomUUID(),name};ext.meow_secondary_profiles.push(p);}Object.assign(p,connectionSnapshot());ext.meow_secondary_profile_id=p.id;save();renderConnections();status('副 API 配置已保存。');});
 on('secondary-profile-list',()=>{const p=ext.meow_secondary_profiles.find(p=>p.id===el('secondary-profile-list').value);if(!p){renderConnections();return;}if(busy){renderConnections();throw new Error('请等待当前任务完成。');}const old=ext.meow_secondary_profiles.find(p=>p.id===ext.meow_secondary_profile_id);if((el('secondary-key').value.trim()||!old||Object.keys(connectionSnapshot()).some(k=>old[k]!==secondary[k]))&&!confirm('切换副 API？当前未存入配置的修改将被替换。')){renderConnections();return;}Object.assign(secondary,{url:p.url,model:p.model,secret_id:p.secret_id});ext.meow_secondary_profile_id=p.id;el('secondary-profile-name').value=p.name;el('secondary-url').value=p.url;el('secondary-model').value=p.model;el('secondary-key').value='';el('secondary-model-list').replaceChildren(new Option('拉取后选择模型',''));el('models-state').textContent='已切换配置，可重新拉取模型。';save();renderConnections();destination();keyStatus();status(`已切换副 API：${p.name}`);},'change');
 on('secondary-profile-delete',()=>{if(busy)throw new Error('请等待当前任务完成。');const id=el('secondary-profile-list').value,p=ext.meow_secondary_profiles.find(p=>p.id===id);if(!p||!confirm(`删除配置“${p.name}”？当前连接和已保存密钥会保留。`))return;ext.meow_secondary_profiles=ext.meow_secondary_profiles.filter(p=>p.id!==id);ext.meow_secondary_profile_id='';save();renderConnections();status('已删除配置；当前连接仍可使用。');});
...
`;
 ...
 async function drawSelection({text,messageIndex}){
  ...
  const part=selectionPart(text,messageIndex),linked=!!part,selectionKey=chatKey(),sourceMessage=Number.isInteger(messageIndex)?ctx().chat[messageIndex]:null,sourceSnapshot=sourceMessage&&!sourceMessage.is_system?sourceMessage.mes:null,originalContext=sourceSnapshot?stripInline(sourceSnapshot):'';
  ...
  pick.append(title,quote,where,countLabel,row,textNode('small',originalContext?'会发送划线内容及所在消息的完整原文，使用坏猫猫预设生成 tags；以当前场景的服装为准。':'未定位到来源消息，只发送划线内容，使用坏猫猫预设生成 tags。'));
  ...
  if(pick.open)pick.close();if(!choice)return;if(selectionKey!==chatKey()||(sourceSnapshot!==null&&ctx().chat[messageIndex]?.mes!==sourceSnapshot))throw new Error('来源聊天或正文已变化，请重新划线。');
  ...
  panel.open();page('bad');
  await run(async signal=>{await runTags(signal,Math.min(20,Math.max(1,choice.count)),originalContext);status('tags 写好了，开始画图…');await runBadGenerate(signal);});
 }
