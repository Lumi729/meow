...
export async function init(){
 ...
 async function redrawEntry(entry,signal){const payload=reseed(entry);status('正在重绘…');const src=await png(payload,signal);const next=makeEntry(src,payload,entry.source,entry.title,entry.chatKey);next.insertionSource=entry.insertionSource;next.cameraKey=entry.cameraKey;await addImage(next);camera?.notify(next.cameraKey);status('重绘完成，已存入图库。');return next;}
 ...
 actions.append(link,button('重绘（新种子）',()=>run(async signal=>{const next=await redrawEntry(entry,signal);showImage(next.id,viewerOrigin);})),...(linked?[button('插入对应原文',()=>insert(entry))]:[]),button('删除图片',async()=>{if(!confirm('删除图库中的这张图片和对应记录？已插入聊天的副本不会删除。'))return;await removeEntry(entry);}));
 ...
 const loadDraft=()=>{draftKey=chatKey();const d=readDrafts()[draftKey];parts=Array.isArray(d?.parts)?d.parts:[];scenes=Array.isArray(d?.scenes)?d.scenes:[];manualPreview=!!d?.manual;capture=parts.length||manualPreview?{key:chatKey()}:null;renderParts();el('send-preview').value=d?.preview??'';el('raw-tags').value=d?.raw??'';renderScenes();if(scenes.length||parts.length)status(scenes.length?'已恢复上次的 tags，可直接生成图片。':'已恢复上次捕捉的原文。');};
 const selectionPreview=()=>{manualPreview=false;el('send-preview').value=selected().map(p=>`[${p.id}] ${p.name} · ${p.part}\n${p.text}`).join('\n\n');scenes=[];el('scenes').replaceChildren();saveDraft();};
 ...
 on('tags',()=>run(async signal=>{const typed=el('send-preview').value.trim(),manual=manualPreview&&!!typed;if(manual)capture={key:chatKey()};else if(!capture)throw new Error('请先点①捕捉聊天，或直接在「将发送的原文预览」里写内容。');checkChat();if(!secondary.secret_id)throw new Error('请先配置副 API 密钥。');const count=numberIn(secondary.image_count,1,8,'图片数');const withCharacters=!!secondary.character_mode;
