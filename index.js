...
export async function init(){
 ...
 tools=mountTools({root,el,store:galleryStore(`${ext.meow_gallery_scope}:tools`),settings,secondary,advanced,drawFields,save,status,run,getToken,makeEntry,addImage,reverseTags,openTools:section=>{if(viewer.open)viewer.close();panel.open();page('tools');el(`sec-${section}`)?.scrollIntoView?.({block:'start'});}});
